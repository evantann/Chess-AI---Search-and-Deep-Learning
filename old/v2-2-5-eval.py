...
polyglot_filenames = [
    'baron30.bin', 'Book.bin', 'book2.bin', 'codekiddy.bin',
    'DCbook_large.bin', 'Elo2400.bin', 'final-book.bin', 'gm2001.bin',
    'gm2600.bin', 'human.bin', 'komodo.bin', 'KomodoVariety.bin',
    'Perfect2023.bin', 'Performance.bin', 'Titans.bin', 'varied.bin'
]
...
while running and games_played < max_games:
    ...
    if board.turn == chess.BLACK:  # AI plays as Black
        time_limit = 15  # Time limit for iterative deepening in seconds
        book_move = get_book_move(board)
        if book_move:
            ai_move = book_move
            print(f"Black (book) {ai_move.uci()}")
        else:

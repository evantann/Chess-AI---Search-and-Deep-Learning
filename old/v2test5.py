...
def is_endgame(board):
    ...
    return num_pieces < 10  # Simple heuristic: endgame if fewer than 10 pieces remain
...
def iterative_deepening(board, max_depth, time_limit):
    ...
    for depth in range(1, max_depth + 1):
        if time.time() - start_time > time_limit:
            break
        _, move = minimax(depth, False, MIN, MAX, board, start_time, time_limit)
        if move:
            best_move = move

    return best_move

def minimax(depth, maximizingPlayer, alpha, beta, board, start_time, time_limit):
    ...
    ordered_moves = order_moves(board)  # Use the move ordering function

    if maximizingPlayer:
        best = MIN
        for move in ordered_moves:
            board.push(move)
            val, _ = minimax(depth - 1, False, alpha, beta, board, start_time, time_limit)
            board.pop()
            if val > best:
                best = val
                best_move = move
        ...
        for move in ordered_moves:
            board.push(move)
            val, _ = minimax(depth - 1, True, alpha, beta, board, start_time, time_limit)
            board.pop()
            if val < best:
                best = val
                best_move = move
...
def evaluate_board(board):
    ...
    for square, piece in board.piece_map().items():
        if piece.piece_type == chess.KING:
            if is_endgame(board):
                positional += king_endgame_table[square // 8][square % 8] * (1 if piece.color == chess.WHITE else -1)
            else:
                positional += piece_square_table[chess.KING][square // 8][square % 8] * (1 if piece.color == chess.WHITE else -1)
        else:
            positional += piece_square_table[piece.piece_type][square // 8][square % 8] * (1 if piece.color == chess.WHITE else -1)
...
while running and games_played < max_games:
    ...
    if board.turn == chess.BLACK:  # AI plays as Black
        time_limit = 5  # Time limit for iterative deepening in seconds
        book_move = get_book_move(board)
        if book_move:
            board.push(book_move)
            print(f"Black (book) {book_move.uci()}")
        else:
            ai_move = iterative_deepening(board, max_depth, time_limit)
            if ai_move:
                board.push(ai_move)

...
def minimax(depth, maximizingPlayer, alpha, beta, board, ply=0):
    ...
    if maximizingPlayer:
        ...
        for move in ordered_moves:
            board.push(move)
            # Apply LMR: Reduce depth for late moves, but be more conservative
            reduction = 1 if not first_move and depth > 3 and ply > 3 else 0
            val, _ = minimax(depth - 1 - reduction, False, alpha, beta, board, ply + 1)
            board.pop()
            if val > best:
                best = val
                best_move = move
        ...
        for move in ordered_moves:
            ...
            # Apply LMR: Reduce depth for late moves, but be more conservative
            reduction = 1 if not first_move and depth > 3 and ply > 3 else 0
            val, _ = minimax(depth - 1 - reduction, True, alpha, beta, board, ply + 1)
            board.pop()
            if val < best:
                best = val
                best_move = move

...
def minimax(depth, maximizingPlayer, alpha, beta, board, start_time, time_limit):
    board_key = hash(board.fen())
    if board_key in transposition_table and transposition_table[board_key][0] >= depth:
        return transposition_table[board_key][1], transposition_table[board_key][2]
    ...
    if maximizingPlayer:
        ...
        for move in board.legal_moves:
            ...
            if beta <= alpha:
                break
        transposition_table[board_key] = (depth, best, best_move)
...
while running and games_played < max_games:
    ...
    if board.turn == chess.BLACK:  # AI plays as Black
        time_limit = 10  # Time limit for iterative deepening in seconds

...
def evaluate_endgame(board):
    material = sum(get_piece_value(piece) for piece in board.piece_map().values())
    king_activity = sum(piece_square_table[chess.KING][square // 8][square % 8] * (1 if piece.color == chess.WHITE else -1) for square, piece in board.piece_map().items() if piece.piece_type == chess.KING)
    return material + king_activity
...
def evaluate_board(board):
    ...
    opening_score = evaluate_opening(board)

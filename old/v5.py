...
# List of Polyglot book filenames
polyglot_filenames = [
    'varied.bin',
    'Performance.bin',
    'KomodoVariety.bin',
...
# Generate the full paths to the Polyglot books
...
def Max(state, depth, alpha, beta):
    ...
    for move in state.legal_moves:
        ...
        evaluation, _ = Min(state_copy, depth + 1, alpha, beta)
        if evaluation > val:
            val = evaluation
            best_move = move
        alpha = max(alpha, val)
...
def Min(state, depth, alpha, beta):
    ...
    for move in state.legal_moves:
        state_copy = state.copy()
        state_copy.push(move)
        evaluation, _ = Max(state_copy, depth + 1, alpha, beta)
        if evaluation < val:
            val = evaluation
            best_move = move
        beta = min(beta, val)
...
def ai_move_logic():
    ...
    if polyglot_move and polyglot_move in board.legal_moves:
        ...
        _, ai_move = Min(board.copy(), 0, -float('inf'), float('inf'))  # Use a copy of the board for evaluation
        if ai_move and ai_move in board.legal_moves:
            board.push(ai_move)
            print("AI moved:", ai_move)
        else:
            print("No legal move found by AI")

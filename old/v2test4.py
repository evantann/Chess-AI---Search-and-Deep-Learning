...
def minimax(depth, maximizingPlayer, alpha, beta, board):
    if depth == 0 or board.is_game_over():
        return quiescence(alpha, beta, board, 3), None  # Pass a maximum depth limit to the quiescence function
...
def quiescence(alpha, beta, board, depth, max_quiescence_depth=3):
    """
    Quiescence search to evaluate positions only involving captures and other tactical moves.
    """
    ...
    for move in ordered_moves:
        if board.is_capture(move):
            board.push(move)
            score = -quiescence(-beta, -alpha, board, depth + 1, max_quiescence_depth)
            board.pop()

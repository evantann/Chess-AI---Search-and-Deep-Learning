...
def evaluate_board(board):
    ...
    if is_endgame(board):
        positional = sum(piece_square_table["ENDGAME_KING"][square // 8][square % 8] * (1 if piece.color == chess.WHITE else -1) for square, piece in board.piece_map().items() if piece.piece_type == chess.KING)
        ...
        pawn_structure = 0
    return material + positional + pawn_structure
    ...
    for square, piece in board.piece_map().items():
        if piece.piece_type == chess.PAWN:
            if board.turn == chess.WHITE:
                score += evaluate_pawn(board, square, piece)
            else:
                score -= evaluate_pawn(board, square, piece)
    return score

def evaluate_pawn(board, square, piece):
    score = 0
    ...
    if file > 0 and not board.pieces(chess.PAWN, piece.color) & chess.BB_FILES[file - 1] and file < 7 and not board.pieces(chess.PAWN, piece.color) & chess.BB_FILES[file + 1]:
        score -= 20
    # Penalty for doubled pawns
    if len(board.pieces(chess.PAWN, piece.color) & chess.BB_FILES[file]) > 1:
        score -= 10
    return score

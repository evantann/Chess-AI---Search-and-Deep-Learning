...
def evaluate_board(board):
    ...
    positional = sum(
        piece_square_table[piece.piece_type][square // 8][square % 8] * (1 if piece.color == chess.WHITE else -1)
        for square, piece in board.piece_map().items()
    )

    # Mobility evaluation
    white_mobility = len(list(board.legal_moves))
    board.turn = not board.turn  # Switch turn to calculate opponent's mobility
    black_mobility = len(list(board.legal_moves))
    board.turn = not board.turn  # Switch turn back

    mobility = (white_mobility - black_mobility) * 10
